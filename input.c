...
static int read_destination(Move *m) {
    ...
    if (dest_col >= 'a' && dest_col <= 'z') //accepting lowercase inputs
...
//This will build the menu of movable pieces for the crurrent player, enorcing jump rule
//Will retun  the valid pieces, so when its 0 a winner is delcared
